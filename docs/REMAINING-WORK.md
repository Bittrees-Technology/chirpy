...
12. **Email and wallet interoperability:** recipient routing, external email composition, web wallet invitation links and the Channels view are implemented. A wallet-signed outbound email pilot is implemented but disabled pending verified consent, provider secrets/domain, scheduler and operator acceptance. Authenticated worker heartbeat/backlog checks are implemented; external monitoring schedules and alert destinations still need provisioning; see [WALLET-EMAIL.md](WALLET-EMAIL.md). Inbound email→wallet forwarding, verified email onboarding, full bridge history, automated abuse/suppression handling and an in-app email inbox remain unfinished. Configuration defaults and acceptance gates are in [MESSAGE-ROUTING.md](MESSAGE-ROUTING.md). Native mailto/universal-link acceptance remains required.
