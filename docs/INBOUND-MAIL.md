...
## Original-recipient forwarding history

New intake records include an opaque `historyOwner` tag derived by domain-separated HMAC from the inbound data key and the wallet returned by the trusted Wallet authority. A private sorted index holds opaque job keys for that owner. No plaintext recipient wallet or mailbox is added to retained metadata. Queue metadata already retains receipts for 30 days; this feature does not extend that retention or retain message payloads longer. The original wallet can inspect its historical status after payload deletion or binding revocation; rebinding the mailbox to another wallet does not transfer old history or grant mailbox access.

`GET /api/mail/inbound-history` advertises version1 only when the existing inbound configuration is enabled and valid in production. `POST` requires the separate **Chat incoming forwarding history v1** wallet signature, binding the fixed `https://chat.bittrees.org/api/mail/inbound-history` service, wallet, random nonce, cursor and at most five-minute expiry. Mailbox cookies and outbound signatures grant no access. The existing dynamic Mail function dispatches this one action separately; connected-mail challenge, callback and operation guards remain unchanged, and browser access to the HMAC inbound intake route remains denied.

CORS permits the Chat origin automatically. Configure `CHAT_MAIL_INBOUND_HISTORY_ALLOWED_ORIGINS` explicitly for Chirpy and approved native origins before accepting those clients. Invalid origin configuration fails closed. Native clients use their configured API origin for transport while signing the fixed Chat history identity. Previews and disabled services cannot expose history. API reads are rate-limited, no-store, signature-checked before storage and expiry-checked again using Redis time.

Each signed page reads at most25 retained records plus one index lookahead and returns only bridge reference, queue state, creation/recorded-transition timestamps, authorization deadline and processing claim count. Claims are not SDK sends. Published means the bridge recorded publication evidence, never recipient delivery/read. Uncertain and stopped states do not justify automatic resending. No retry, reset or requeue action is exposed. Reads do not touch job TTL, queue state, sender journal or encrypted payloads. Missing rows are omitted, mismatched ownership/corrupt records fail closed, and a removed cursor requires a fresh lookup.

The index is capped at32,000 recent entries, above the maximum expected within30-day retention under the global1,000-per-day intake limit. Expired pointers are pruned on insertion/lookup; index expiry follows the newest enqueue. Individual job retention is unchanged. The UI signs each page separately, clears snapshots on wallet/provider events, aborts late results, bounds response size and stores no history in localStorage. English/Spanish labels distinguish retained evidence from current binding or delivery authority.

Legacy receipts without ownership tags are not silently assigned to a wallet and do not appear. The encrypted recipient may already have been deleted; current bindings cannot safely reconstruct historical ownership. Keep existing operator journal/reconciliation procedures for those records. Rotating the inbound data key changes history tags: plan and verify any history migration while the previous key and authoritative records remain available, without rewriting jobs from current mailbox ownership. This increment neither backfills ambiguous records nor changes custody/retention policy. Roll out the updated receiver and worker before live acceptance; older workers may lack claim transition timestamps. Test original and reassigned wallets, revocation, payload deletion, missing/expired records, uncertainty and actual cross-origin/native clients before declaring production history complete.
