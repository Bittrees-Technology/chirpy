...
Initial load, periodic visible polling, focus/online/visibility recovery, room admission, stream errors/retries and failed mapping still require full reconciliation. Unsubscribing disables the incremental path. The healthy-stream polling interval is described below; initial and periodic full synchronization, local full-list enumeration/sorting and real-device large-inbox benchmarks remain unfinished.
...
## Healthy-stream reconciliation cadence
...
## Versioned 1 MiB email transfers (23 September 2026)

The Email composer now permits up to four files totaling 1 MiB, with complete downloads up to 1 MiB per file and original MIME messages up to 2 MiB. Updated Chat explicitly selects transfer version 2; the already-published Mail v54 and matching Acer readers preserve legacy defaults for older clients. Read/send grants, exact source versions, full-file hashes, stale-session cancellation and uncertain-send receipts remain enforced. AI selected reads retain their old limits.

Maximum-size browser tests cover selection/removal, exact downloads, hash rejection, cancellation and uncertain sends. Mail tests found and fixed a same-base64-length one-byte overflow in result storage and reject duplicate version selectors. The actual Python reader's 1 MiB response passes the actual Chat client, and Chat-prepared bytes pass the actual Mail v2 validator while exceeding its legacy limit. These fixture checks complement the previously approved live 256 KiB acceptance; they do not establish a live larger-file round trip, physical-device performance or production forwarding readiness.
...
### Open XMTP room members

Chat-created open rooms expose a member list and an administrator Add member control. Each addition takes one Ethereum wallet that already has an XMTP inbox. The transport syncs the room and reads a fresh SDK wrapper, checks valid metadata and the current organization namespace, refuses gated rooms (including independently published directory gates, with a fresh catalog read), resolves the recipient inbox, rejects self/duplicate members, and rechecks membership, administrator role and the connected wallet before dispatch. Room/wallet/organization changes and duplicate submissions cancel pending work before dispatch; an already-dispatched SDK operation cannot be undone by navigation. Failed requests retain the recipient for review/retry.

This control does not change existing group permission policies or grant an administrator role to the recipient. Other XMTP clients remain subject to the group's underlying SDK permission policy, which can differ from Chat's administrator-only UI. Gated admission stays with the gate service. Demo additions only update local sample data. Production old/new-origin, gated-room and physical-device acceptance remain separate requirements.

Validation includes stale metadata/role and account/session tests, missing/foreign/gated rooms, registration and duplicate checks, SDK failure handling, delayed/duplicate UI submission, and the opt-in disposable-wallet XMTP-dev group messaging/fresh-installation history scenario. Local acceptance passed the web/transport suite, 72 standard browser cases and the actual disposable-wallet XMTP-dev scenario: activated recipient addition, two-way group messages and recovered DM/group history plus membership on a fresh installation. Membership events no longer blank room previews; previews query the latest Text/Reply. Hosted checks and production/device acceptance must still be verified for the release.


### Group author identities

Group previews, history, reactions and member rosters resolve wallet identities from SDK inbox states matched by explicit inbox ID. Reordered, missing, duplicate or foreign records cannot borrow another member's identity. Prefer an active Ethereum recovery identifier; otherwise use only an unambiguous Ethereum identifier. A departed member can still resolve from authenticated inbox state; unknown or ambiguous identities retain their original inbox ID. Message/reaction authority continues to use the original SDK sender inbox ID, independently of the displayed wallet/public label.

Lookups share a memory-only resolver per SDK client: at most 1,000 unique IDs per read, 100 per request, 4 network requests at once, 2,500 pending identities and 2,500 cache entries. Successful records expire after 30 seconds; confirmed lookup misses/failures after 5 seconds. History falls back after 8 seconds of waiting; a slow underlying request retains its slot and shared fallback, preventing repeated polling from multiplying network work. A replacement client cannot reuse the previous client's cache, and a result from a changed client cannot publish history. Raw identities remain readable during failures and limits. Public profile publication/withdrawal is still separately scoped and verified by the profile service.

Local acceptance includes identity substitution/ambiguity, bounds/concurrency/coalescing, cache expiry/failure/slow-response behavior, replaced clients and original reaction authority. The disposable XMTP-dev browser scenario covers wallet-linked group author labels, creator and invited-member DM/group recovery on fresh installations, and synthetic public-label fixtures; it does not publish real profiles or establish physical-device/production-origin acceptance.


The completed local run passed 676 web/transport tests, all 72 standard browser cases, types/build, and the actual two-wallet XMTP-dev scenario in 2.9 minutes: group sender identities and chosen labels, native reaction delivery, synthetic label withdrawal, and DM/group history plus membership recovery for both creator and invited member. Hosted release checks and actual production/device acceptance remain separate gates.


### XMTP consent ordering

The browser SDK is pinned to `7.0.0` with a reviewed local `1.10.0` WASM source patch. Consent updates store state and timestamp together; synchronized records use an atomic strictly-newer replacement, including newer records with the same state. A local same-state no-op does not silently acquire a timestamp that was never broadcast. Creator welcomes supply only a lowest-precedence default, preserving explicit choices regardless of welcome/archive arrival order. Duplicate-DM stitching copies the newest stored choice with its original timestamp. Inbox enumeration explicitly includes Unknown, Allowed and Denied so blocked conversations remain discoverable. A consumed consent listener invalidates summaries on device preference changes, retries failures, and tears down obsolete sessions. In-flight inbox reads drain consent invalidations before returning.

The upstream implementation reproduces older Allowed undoing a newer Block in a real disposable-wallet browser test, as well as three independent SQLite timestamp regressions. Patch coverage includes all 162 three-record state/order combinations, creator defaults before/after explicit choices (including zero timestamps), implicit send-time no-ops, and newest-first DM stitching without redating decisions. CI verifies source/archive/installed-package hashes and runs the consent database tests against the exact upstream commit plus patch. Rebuild instructions, license and provenance are in [the SDK patch guide](../vendor/xmtp-consent/README.md). Real XMTP-dev block/unblock, creator recovery and two-installation synchronization acceptance must pass for the final artifact before deployment.

This does not make blocking a membership removal operation. A blocked conversation recovered from an archive can be inactive on the new installation, and local history readability must not depend on a successful network sync for that inactive room. Native group recipient controls, truthful inactive/recovery/leave states and active-membership restoration remain separate work. Unupdated installations retain their old SDK behavior; existing incorrectly dated records cannot be reconstructed from state alone. A new explicit choice uses the repaired ordering. The temporary patch must be re-reviewed or removed when upgrading the browser SDK.

Recovered DMs resolve missing local participant identities through bounded, keyed SDK network lookups. A missing peer never implies Saved Messages. Local validation passed 690 web/transport tests, 201 applicable SDK database tests (8 upstream ignored), and type checks. The real disposable-wallet dev test passed block/unblock/reblock, creator installation recovery after a new welcome, and subsequent choices synchronized in both directions. Final hosted checks and the full messaging/recovery suite remain release gates.

### Wallet-scoped DM identity cache

Persistent peer labels are scoped to the current wallet and XMTP environment. Switching between participants in the same conversation cannot reuse the other wallet's peer as self. The old unscoped cache is left untouched but is not trusted because it lacks wallet provenance. Scoped cache reads validate wallet strings, entry shape and input size; memory and persisted entries are capped at 2,500. Cache access failures do not prevent messaging. This affects display hints only; SDK inbox IDs remain message authority.

### Native group invitation consent

Native XMTP group invitations appear in Requests and require explicit acceptance before sending, reacting, marking messages read or adding members. Groups can be blocked and unblocked through SDK conversation consent. Blocked rooms stay discoverable in Blocked while previews, loaded messages, profile lookups, member lists and reply previews are hidden. User-owned drafts are retained. Groups do not send network read receipts. Creating a group explicitly accepts only an unknown new group's consent; discovering a group or holding an administrator role never auto-accepts it.

Consent controls apply to an existing native conversation. Unjoined organization-directory entries retain their gate admission flow, and Push membership remains with its own protocol. Directory overlays preserve existing consent. Blocking is a visibility/send preference and does not leave a group, change its permission policy or stop protocol-level delivery. Leave requests, confirmed removal and owner transfer remain separate work.

History, preview and action paths recheck current SDK consent after asynchronous reads; sends/reactions recheck immediately before dispatch. Account/client/room/organization changes reject pending consent actions, duplicate consent mutations are refused and pending mutations prevent sends. Failed SDK writes do not claim success. A consent change during a slow directory fetch drains the invalidation before the older inbox snapshot can publish. Operations already dispatched to the SDK cannot be recalled by navigation or blocking.

Validation covers unknown/denied/unavailable consent, delayed history/preview/identity/policy/reply/reaction work, wallet and scope changes, duplicate/failed writes, directory overlays and unjoined rooms, immediate UI masking, draft preservation and filter isolation. The opt-in disposable XMTP-dev scenario additionally tests invitation acceptance, block/unblock and blocked-consent recovery on a fresh invited-member installation; record the live run result separately before release. Production-origin and physical-device acceptance remain outstanding.

Native group summaries distinguish active, inactive and unavailable device access. Restored local history remains readable after unblock even when that installation is inactive; confirmed inactivity permits reading without a failing network sync. Genuine network failures for an active group still surface. Sending, reactions, member additions and policy changes require active access immediately before dispatch, independently of consent and administrator role. The UI disables these actions and labels inactive membership snapshots as a saved member list. Unblock does not itself restore access; an existing member's membership update must activate the installation before sending is available again.
...
## Deployed Chirpy/Chat encrypted settings continuity
