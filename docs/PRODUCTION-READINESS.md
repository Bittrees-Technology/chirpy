...
Saved-message duplicates use their own update time and a deterministic content tie-breaker independent of JSON object key order. Legacy messages receive a persisted fallback time from their original snapshot, so later unrelated settings updates cannot make old content appear newer. Stable output ordering prevents device-dependent array order. Regression tests reproduce four failures in the previous merge and check opposite merge directions, three-device exchange orders, JSON round trips, duplicate IDs, non-mutation and conservative receipt removal conflicts.
