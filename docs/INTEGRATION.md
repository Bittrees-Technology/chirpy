...
| Local data | Reviewed encrypted settings/contact/note/private-name export and restore; Governance source export | Governance desktop export/restore accepted on deployed origins; Research integration/export, physical/native devices and actual messaging history remain |
...
Governance PR24 makes local recovery export available before XMTP activation. Actual Governance→Chat desktop-browser contacts/notes export, reviewed preference consent, wrong-passphrase rejection, encrypted backup and undo passed23September2026 using isolated synthetic local stores and the deployed applications. See [the deployed recovery procedure and limits](DEPLOYED-RECOVERY-ACCEPTANCE.md). This does not satisfy Research, physical/native device or protocol-history acceptance.
