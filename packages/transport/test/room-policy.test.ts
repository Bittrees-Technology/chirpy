...
function setup() {
  ...
  const room = { isPendingRemoval: vi.fn().mockResolvedValue(false), isActive: vi.fn().mockResolvedValue(true), consentState: vi.fn().mockResolvedValue(1), id: 'room', name: 'Room', isAdmin: vi.fn(async () => false), isSuperAdmin: vi.fn(async () => false), sendReaction: vi.fn(), updateDescription: vi.fn() };
