...
Chat is the new product name. The move to `chat.bittrees.org` is being prepared;
existing URLs and internal compatibility identifiers remain in use during the transition.

Chat is an **org-agnostic** chat client for communities that organize around wallets.
...
primary frame: Chat is meant to be reusable by any wallet-native community
...
- **User and value:** Chat serves wallet-native communities that need encrypted direct
...
- **Out of scope:** Chat does not provide governance, treasury, membership issuance, or the
...
- **Trust and support ownership:** Chat must own approved support, security, privacy, and
...
- **No sibling dependency:** building, previewing, and using Chat does not require a Bittrees
...
- **Optional integration boundary:** presets, wallet/XMTP mode, ENS, hosted KV sync, external
  ...
  they are absent; enabling an integration does not make a sibling product part of Chat.
...
pnpm tauri build        # build Chat.app / .dmg
...
- ✅ **Action policy** layer (read-only rooms, block attachments, size caps) — pause posting in Chat; this is advisory client policy.
