...
After reviewing the package and host preflight, run the same pinned installer with administrator privileges, omitting `--verify-only`. It creates no-login `chat-mail-inbound`, root-owned code in `/opt/chat-mail-inbound`, configuration in `/etc/chat-mail-inbound`, and an empty private writable parent `/var/lib/chat-mail-inbound`. `worker.env` is root-only; disabled `source.json` is owned by the dedicated service user with mode0400, beneath the root-owned directory. Mail requires a private file; `ProtectSystem=strict` prevents the service from changing its contents or permissions. The final sender child path remains absent for exclusive provisioning. No packaged executable is run by the administrator installer.
...
The sender provisioning request uses `/etc/chat-mail-inbound/source.json`. Keep that file0400 and owned only by the dedicated service user, with its parent root-owned0750. Both worker and health units mount configuration read-only through `ProtectSystem=strict`; the hosted kernel probe verifies that neither writes nor permission changes are allowed. Existing installations require a reviewed ownership/mode update; never rerun the first-install-only installer over them.
