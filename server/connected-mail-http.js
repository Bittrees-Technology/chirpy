...
export function createConnectedMailHandler({config=action=>connectedMailConfig(process.env,{allowDisabled:action==='disconnect'}),service=c=>createConnectedMail(c),rate=checkRateLimit}={}){
 return async(req,res)=>{
  ...
  try{
   ...
   if(Buffer.byteLength(typeof req.body==='string'?req.body:JSON.stringify(req.body??{}))>(action==='operation'&&req.body?.input?.transferVersion===2?1450000:action==='operation'?410000:65536))throw new ConnectedMailError(413,'Mail request is too large.');
   let input=req.body;
