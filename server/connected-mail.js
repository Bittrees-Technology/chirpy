...
export function createConnectedMail(config,{kv=connectionKv(config),request=fetch,verify=(wallet,message,signature)=>verifyWallet(config,wallet,message,signature),now=()=>Date.now()}={}){
 ...
 return {
  ...
  async callback(token,input){
   ...
   if(!hex(grant?.token)||!hex(grant?.grantId)||grant.wallet!==input.wallet||grant.audience!==CHAT_ORIGIN||typeof grant.mailbox!=='string'||!/^[a-z0-9][a-z0-9._-]{0,63}@bittrees\.org$/.test(grant.mailbox)||!Array.isArray(grant.scopes)||!grant.scopes.length||grant.scopes.length>2||new Set(grant.scopes).size!==grant.scopes.length||grant.scopes.some(s=>!['read','send'].includes(s))||!Number.isFinite(expiry)||expiry<=now()||expiry>now()+3600000){if(hex(grant?.token))await revokeGrant(grant.token);throw new ConnectedMailError(502,'Mail returned an invalid connection.');}
  ...
  async operation(token,input){
   ...
   const scope=input.action==='send'?'send':['folders','messages','message'].includes(input.action)?'read':null;
