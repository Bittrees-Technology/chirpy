...
vi.mock("@app/transport", async (importOriginal) => ({ ...await importOriginal<typeof import("@app/transport")>(), createTransport: () => ({ id: "mock", status: "ready", init: async () => {}, listConversations: async () => [], listMessages: async () => [], listMessagePage: async () => ({ messages: [] }), subscribe: () => () => {}, markRead: mock.markRead }) }));
...
it("passes the actual current Settings preference to the transport", async () => {
  ...
  try {
    ...
    await act(async () => chat.markRead("seen"));
    expect(mock.markRead).toHaveBeenLastCalledWith("dm", { sendReceipt: true, throughMessageId: "seen" });
    await act(async () => settings.pauseSyncForRecovery());
    await act(async () => chat.markRead("seen"));
    expect(mock.markRead).toHaveBeenLastCalledWith("dm", { sendReceipt: false, throughMessageId: "seen" });
    await act(async () => settings.refreshAfterRecovery());
    await act(async () => chat.markRead("seen"));
    expect(mock.markRead).toHaveBeenLastCalledWith("dm", { sendReceipt: true, throughMessageId: "seen" });
