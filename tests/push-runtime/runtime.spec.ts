...
test('real browser SDK imports, crypto and signer compatibility without external traffic or persistent keys', async ({ page, baseURL }) => {
  ...
  expect(result).toMatchObject({ sdkLoaded: true, maxBatchCrypto: true, isolatedRoomKeys: true, realRuntimeReady: true, withoutInjectedReady: true, withoutInjectedPublicKey: true, legacyRecoveryBound: true, decrypted: 'Synthetic room message', tamperedRejected: true, staleRejected: true, personalSigned: true, typedSigned: true, storageUnchanged: true });
