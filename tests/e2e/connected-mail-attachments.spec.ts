...
test('connected email sends the selected binary file once and guards uncertain delivery',async({page,walletAddress},testInfo)=>{
 ...
 let releaseRefresh!:()=>void;const refresh=new Promise<void>(resolve=>{releaseRefresh=resolve;});
 ...
 await page.route('**/api/mail/**',async route=>{
  ...
  if(body.action==='messages'){if(sends.length===1&&!uncertain)await refresh;return route.fulfill({json:{messages:[],nextCursor:null}});}
