...
test('a complete attachment uses one read, shows progress, and saves only verified bytes',async({page,walletAddress},testInfo)=>{
 const wallet=walletAddress.toLowerCase(),id='a'.repeat(64),version='b'.repeat(64),bytes=Buffer.alloc(262144,37),sha256=createHash('sha256').update(bytes).digest('hex');
 ...
 await page.route('**/api/mail/**',async route=>{
  ...
  if(request.action==='attachments')return route.fulfill({json:{id,sourceVersion:version,chunkBytes:12288,maxAttachmentBytes:262144,attachments:[file]}});
  expect(request.action).toBe('attachmentFile');expect(request.input).toEqual({folder:'INBOX',id,version,part:'1.2'});reads++;
  if(mode==='hold')await new Promise<void>(r=>release=r);
  await route.fulfill({json:{id,sourceVersion:version,transfer:'complete',maxAttachmentBytes:262144,attachment:file,data:bytes.toString('base64'),sha256:mode==='bad'?'c'.repeat(64):sha256}}).catch(()=>{});
 });
 ...
 await expect(page.getByRole('progressbar',{name:'Attachment download progress'})).toBeVisible();await expect(page.getByRole('status')).toContainText('Preparing fixture.bin (262144 bytes)');await expect.poll(()=>typeof release).toBe('function');
