...
if(process.env.CHAT_PRODUCTION_RECOVERY_ACCEPTANCE!=='1'||!/^[a-f0-9]{40}$/.test(process.env.CHAT_EXPECTED_SHA??'')||!/^[a-f0-9]{64}$/.test(process.env.GOV_EXPECTED_ASSET_SHA??''))throw Error('Explicit deployed recovery acceptance configuration required.');
const gov='https://gov.bittrees.org',chat='https://chat.bittrees.org';
...
const contacts=[{address:peer,label:'Synthetic Governance contact'}],notes=[{id:'synthetic-gov-note',text:'Synthetic Governance local note',sentAtMs:123}];
const original={
...
async function versions(){
 ...
 const response=await fetch(gov+'/messenger',{redirect:'error',signal:AbortSignal.timeout(15000)});expect(response.status).toBe(200);
 const html=await response.text(),path=/<script\b(?=[^>]*type="module")[^>]*src="([^"]+)"/.exec(html)?.[1];expect(path).toBeTruthy();
 const url=new URL(path!,gov);expect(url.origin).toBe(gov);const asset=await fetch(url,{redirect:'error',signal:AbortSignal.timeout(15000)});expect(asset.status).toBe(200);
 expect(createHash('sha256').update(Buffer.from(await asset.arrayBuffer())).digest('hex')).toBe(process.env.GOV_EXPECTED_ASSET_SHA);
}
async function inject(context:BrowserContext,origin:string,signatures:string[],writes:string[]){
 await context.route('**/*',route=>{
  const request=route.request(),url=new URL(request.url());
  if([gov,chat,'https://chirpy.bittrees.org'].includes(url.origin)&&!['GET','HEAD'].includes(request.method()))writes.push(url.pathname);
 ...
 await context.addInitScript(({address,origin,original})=>{
  if(location.origin===origin&&origin==='https://gov.bittrees.org')for(const [key,value]of Object.entries(original))localStorage.setItem(key,value);
  let connected=false;
 ...
 },{address:account.address,origin,original});
}
...
async function upload(page:Page,raw:string,pass=password){
 await page.getByLabel('Encrypted recovery file',{exact:true}).setInputFiles({name:'governance-recovery.json',mimeType:'application/json',buffer:Buffer.from(raw)});
...
test('deployed Governance export restores locally in Chat with separate preference consent, backup and undo',async({browser},info)=>{
 const contexts:BrowserContext[]=[],signatures:string[]=[],writes:string[]=[];await versions();
 try{
  const source=await browser.newContext(),target=await browser.newContext();contexts.push(source,target);await inject(source,gov,signatures,writes);await inject(target,chat,signatures,writes);
  const one=await source.newPage();await one.goto(gov+'/messenger');await one.getByRole('button',{name:'Connect Wallet',exact:true}).first().click();await one.getByRole('button',{name:'MetaMask',exact:true}).click();
  ...
  const decoded=await decryptRecoveryArchive(raw,password,owner);expect(decoded.source).toBe('governance');expect(decoded.contacts).toEqual(contacts);expect(decoded.notes).toEqual(notes);expect(JSON.stringify(decoded)).not.toContain(original['synthetic-secret-canary']);
  ...
  await two.setViewportSize({width:390,height:844});await two.getByRole('heading',{name:'Review proposed changes'}).scrollIntoViewIfNeeded();expect(await two.evaluate(()=>document.documentElement.scrollWidth<=innerWidth)).toBe(true);await two.screenshot({path:info.outputPath('live-governance-restore-mobile.png')});
  ...
  console.log(JSON.stringify({wallet:owner,chatSha:process.env.CHAT_EXPECTED_SHA,governanceAssetSha:process.env.GOV_EXPECTED_ASSET_SHA,deployedExportRestore:true,separateConsent:true,wrongPasswordRejected:true,backupAndUndo:true,sourcePreserved:true,applicationNetworkWrites:0}));
