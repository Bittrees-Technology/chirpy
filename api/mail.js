...
export default async function handler(req,res) {
  ...
  if(req.method==='GET') return res.status(200).json({enabled:true,service:config.service,authVersion:1,historyVersion:1});
  ...
  try {
    ...
    const status={denied:403,conflict:409,'history-changed':409,limited:429,expired:401}[result.status]||200;
